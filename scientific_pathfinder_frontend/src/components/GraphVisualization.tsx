...
export default function GraphVisualization({ sessionId, onClose }: Props) {
  ...
  const fetchGraphData = async () => {
    setLoading(true);
    try {
      // Fetch graph data from backend - use correct endpoint
      ...
      if (data && Array.isArray(data.nodes) && Array.isArray(data.links)) {
        console.log(`Fetched ${data.nodes.length} nodes and ${data.links.length} links`);
        setGraphData(data);
      } else {
        console.warn('Invalid graph data structure, using mock data');
